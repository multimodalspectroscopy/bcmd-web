from new_code.bcmd_model import ModelBCMD
...
def test_default_output():
    ...
    assert_true(filecmp.cmp(default_model.output_coarse, actual), msg='Coarse output files do not match actual.')
...
def test_output_dict():
    ...
    np_test.assert_almost_equal(vc_test,[0.99326201853524232, 0.0066925479138209504,
                                 -0.99321690724584777, -0.0066922439555526904,
                                 0.99321692632751313], err_msg='Vc Output not the same')
    os.remove(default_model.output_coarse)
    os.remove(default_model.output_detail)
